...
# change name to "mid_semeseter.csv"
...
# import 
data = read.csv(file.path(getwd(), "only_mike", "mid_semester.csv")) %>%
  
  slice(-c(2,1)) %>% # remove 2 and 3 rows (questions description) 
...
rio::export(final_table, file.path(getwd(), "only_mike", "mid_weight.csv"))
